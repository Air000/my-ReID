...
from .bases import TextImageDataset
from timm.data.random_erasing import RandomErasing
from .sampler import RandomIdentitySampler
from .sampler_ddp import RandomIdentitySampler_DDP
import torch.distributed as dist
from .t2i_veri import T2I_VeRi
...
def train_collate_fn(batch):
    ...
    """
    imgs, pids, caps, _ = zip(*batch)
    pids = torch.tensor(pids, dtype=torch.int64)
    return torch.stack(imgs, dim=0), pids, caps

def val_collate_fn(batch):
    imgs, pids, caps, img_paths = zip(*batch)
    return torch.stack(imgs, dim=0), pids, caps, img_paths
...
def make_dataloader(cfg):
    ...
    train_set = TextImageDataset(dataset.train, train_transforms)
    train_set_normal = TextImageDataset(dataset.train, val_transforms)
    num_classes = dataset.num_train_pids
    cap_num = dataset.num_train_caps
    ...
    val_set = TextImageDataset(dataset.query + dataset.gallery, val_transforms)
    ...
    return train_loader_stage2, train_loader_stage1, val_loader, len(dataset.query), num_classes, cap_num
